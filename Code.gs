 ...
 */
function onOpen() {
  var menu = [{name: 'Set up workshop', functionName: 'setUpWorkshop_'}];
  SpreadsheetApp.getActive().addMenu('Workshop', menu);
}
 ...
 */
function setUpWorkshop_() {
 // if (ScriptProperties.getProperty('calId')) {
   // Browser.msgBox('Your workshop is already set up. Look in Google Drive!');
//  }
...
function setUpCalendar_(values, range) {
  var cal = CalendarApp.createCalendar('Workshop Calendar');
...
function setUpForm_(ss, values) {
  ...
  var form = FormApp.create('Workshop Scheduler');
  form.setDescription('List of all workshops getting held in HUGE Tech');
  ...
  form.addSectionHeaderItem().setTitle('Due to limited seats, Please select only those which you can attend!')
...
function onFormSubmit(e) {
  var user = {name: e.namedValues['Name'][0], email: Session.getActiveUser().getEmail()};

  // Grab the session data again so that we can match it to the user's choices.
  var response = [];
  var values = SpreadsheetApp.getActive().getSheetByName('Schedule')
...
function sendDoc_(user, response) {
  var doc = DocumentApp.create('Workshop Schedule for ' + user.name)
  ...
  MailApp.sendEmail({
    to: user.email,
    subject: doc.getName(),
    body: 'Thanks for registering! Here\'s your workshop schedule: ' + doc.getUrl(),
